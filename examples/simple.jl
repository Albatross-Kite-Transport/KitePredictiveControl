...
# --- user constants ---
plt = 0 # 0 = none, 1 = graph, 2 = 2d, 3 = 3d
Ts = 0.1
...
if (plt==3) viewer::Viewer3D = Viewer3D(true) end
...
function wanted_heading(t, pos_y, last_wanted_heading)f
...
total_time = 60
try
    if (plt==1) start_processes!(ci) end
    for i in 1:Int(div(total_time, Ts))
        ...
        # rheading = deg2rad(-90)
        println("MPC step")
        @time set_values = KitePredictiveControl.step!(ci, kite_real.integrator.u, y; rheading)
        update_sys_state!(ss, kite_real, 1.0)
        if (plt==3) update_system(viewer, ss, scale=1.0, kite_scale=1.0) end
        ...
        if (plt==2) plot2d(kite_real.pos, (i-1)*Ts; zoom=true, front=true, xlim=(-l, l), ylim=(0, 2l)) end
    end
finally
    if (plt==1) stop_processes!(ci) end
end